...
class ReimbursementPerjalananBisnisMKExport implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles
{
    use Exportable;
    ...
    public function collection()
    {
        $query = DB::table('tb_reimbursement')
            ...
            GROUP_CONCAT(DISTINCT tb_proyek.nama_proyek SEPARATOR ", ") AS nama_proyek,
            ...
            ->leftJoin('tb_proyek', 'tb_proyek.id_proyek', '=', 'tb_lampiran.id_proyek')
            ->where('tb_jenis_reimbursement.id_jenis_reimbursement', '=', 2)
            ->where('tb_reimbursement.id_status_pengajuan_mk', '!=', null)
            ->where('users.hapus', '=', 0)
            ...
            // ->whereIn('tb_status_pengajuan.id_status_pengajuan', [21,10,9,26,35,8])
            ...
            if ($this->searchValue) {
                ...
                $query->where(function ($query) use ($searchValue) {
                    $query->where('users.nomor_identitas_karyawan', 'like', '%' . $searchValue . '%')
                        ...
                        ->orWhere('tb_proyek.nama_proyek', '=', $searchValue)
                        ->orWhere('tb_proyek.nama_proyek', 'like', '%' . $searchValue . '%')
            ...
            $query->groupBy(
                ...
                DB::raw("(SELECT nama_proyek FROM tb_proyek WHERE tb_lampiran.id_proyek = tb_proyek.id_proyek)"),
                'tb_status_pengajuan.nama_status_pengajuan',
