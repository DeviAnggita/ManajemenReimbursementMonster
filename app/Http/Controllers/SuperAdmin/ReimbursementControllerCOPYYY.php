...
class ReimbursementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //buat
        // $email_karyawan = $user->email_karyawan; //divisi sama , role 4
        ...
        if ($lampiranData) {
            ...
            foreach ($lampiranData as $index => $data) {
                ...
                // // Validasi jika lampiran 2 atau lampiran 3 akan ditambahkan
                // if (($index === 1 || $index === 2) && ($data['nomor_kwitansi'] || $data['tanggal_kwitansi'] || $data['judul_kwitansi'] || $data['nama_kwitansi'] || isset($data['file']) || $data['keterangan'])) {
                //     // Cek apakah semua field form lampiran 2 atau lampiran 3 diisi
                //     if (!$data['nomor_kwitansi'] || !$data['tanggal_kwitansi'] || !$data['judul_kwitansi'] || !$data['nama_kwitansi'] || !isset($data['file']) || !$data['keterangan']) {
                //         // Jika ada satu atau lebih field yang tidak diisi, berikan pesan kesalahan
                //         $lampiranIndex = $index + 1;
                //         return redirect()->back()->withErrors(["Lampiran $lampiranIndex harus mengisi semua field."])->withInput();
                //     }
                // }
                ...
                if ($data['nomor_kwitansi'] || $data['tanggal_kwitansi'] || $data['judul_kwitansi'] || $data['nama_kwitansi'] ||  isset($data['file']) || $data['keterangan']) {
                    ...
                    // Mendapatkan $id_jenis_reimbursement dari $reimbursement
                    $id_jenis_reimbursement = $reimbursement->id_jenis_reimbursement;

                    // Mengakses tabel tb_jenis_reimbursement dan mencari nama_jenis_reimbursement berdasarkan id_jenis_reimbursement dalam tabel tb_reimbursement
